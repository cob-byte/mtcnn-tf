...
class FaceCheck:
    def __init__(self, baby_monitor):
        ...
        self.eyes_open_q = deque(maxlen=4)
...
class MovementTracking:
    def __init__(self, baby_monitor):
        ...
        self.movement_q = deque(maxlen=4) #analyze wrist movements over a specific timeframe (12 secs), a maxlen of 4 (12 seconds / 3 seconds per update)
    ...
    def track_movement(self, frame, left_wrist_coords, right_wrist_coords):
        if self.body_found:  # Use self.body_found here
            ...
            movement_status = "Moving" if len(self.baby_monitor.awake_q) > 0 and sum(self.baby_monitor.awake_q) > 0 else "Not Moving"
        ...
        try:
            while True:
                ...
                if self.video_capture is not None:
                    ...
                    # Only process every 3 seconds or 5 seconds depende pa if iintegrate sa raspberry pi kung ano best
                    if frame_count % (3 * self.video_capture.get(cv2.CAP_PROP_FPS)) == 0:
                        image_rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
                        self.baby_monitor.process_frame(frame, image_rgb)

                        # Save the processed image
                        cv2.imwrite('monitor.jpg')
