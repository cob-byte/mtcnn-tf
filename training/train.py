...
# center_loss
# def train_model(baseLr, loss_softmax_center, loss_qita,  data_num, centers_update_op):
    """
    train model
    ...
    train_op, lr_op
    """
    lr_factor = 0.1 #MomentumOptimizer    
    ...
    # # train_op_softmax = optimizer.minimize(loss, global_step)
    # return train_op, lr_op

    # enter_loss
    # optimizer2 = tf.train.AdamOptimizer(0.001) #   # with tf.control_dependencies([centers_update_op]):
    #     train_op_softmax_center = optimizer2.minimize(loss_softmax_center, global_step=global_step)
    # return train_op_softmax_center, train_op_qita, lr_op

    # #-softmax-loss
    learning_rate = tf.compat.v1.train.exponential_decay(0.001, global_step, 10000, 0.9, staircase=True)
...
def train(netFactory, modelPrefix, endEpoch, dataPath, display=200, baseLr=0.01, gpus=""):
    ...
    if net == 'pnet': # PNet use this method to get data
        ...
        total_num = sum(1 for _ in tf.compat.v1.python_io.tf_record_iterator(dataset_dir))
        ...
        for d in dataset_dirs:
            total_num += sum(1 for _ in tf.compat.v1.python_io.tf_record_iterator(d))
    #ratio 
    if net == 'pnet':
        image_size = 12
    ...
    #define placeholder
    input_image = tf.compat.v1.placeholder(tf.float32, shape=[config.BATCH_SIZE, image_size, image_size, 3], name='input_image')
    label = tf.compat.v1.placeholder(tf.float32, shape=[config.BATCH_SIZE], name='label') #eippo
    # label = tf.placeholder(tf.int64, shape=[config.BATCH_SIZE], name='label')
    bbox_target = tf.compat.v1.placeholder(tf.float32, shape=[config.BATCH_SIZE, 4], name='bbox_target')
    landmark_target = tf.compat.v1.placeholder(tf.float32,shape=[config.BATCH_SIZE,10],name='landmark_target')
    #class,regression
    ...
    # center_loss
    # cls_loss_op,bbox_loss_op,landmark_loss_op,L2_loss_op,accuracy_op, centers_update_op = netFactory(input_image, label, bbox_target,landmark_target,training=True)
    ...
    # train_op_softmax_center, train_op_qita, lr_op = train_model(baseLr, ratio_cls_loss*cls_loss_op,  ratio_bbox_loss*bbox_loss_op + ratio_landmark_loss*landmark_loss_op + L2_loss_op, total_num, centers_update_op)
    # init
    ...
    tf.compat.v1.summary.scalar("bbox_loss",bbox_loss_op)#bbox_loss
    tf.compat.v1.summary.scalar("landmark_loss",landmark_loss_op)#landmark_loss
    tf.compat.v1.summary.scalar("cls_accuracy",accuracy_op)#cls_acc
    summary_op = tf.compat.v1.summary.merge_all()
    logs_dir = os.path.join(rootPath, "tmp", "logs", net)
    if os.path.exists(logs_dir) == False:
        os.makedirs(logs_dir)
    writer = tf.compat.v1.summary.FileWriter(logs_dir, sess.graph)
    #begin 
    ...
    try:
        for step in range(MAX_STEP):
            ...
            #center loss
            # _,_,_,summary = sess.run([train_op_softmax_center, train_op_qita, lr_op ,summary_op], feed_dict={input_image: image_batch_array, label: label_batch_array, bbox_target: bbox_batch_array,landmark_target:landmark_batch_array})
...
if __name__ == "__main__":
    ...
    # print(slim.xavier_initializer())
    # print(tf.keras.initializers.he_normal())
    # input()
    args = parse_args()
