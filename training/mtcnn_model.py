...
#define prelu
def prelu(inputs):
    alphas = tf.compat.v1.get_variable("alphas", shape=inputs.get_shape()[-1], dtype=tf.float32, initializer=tf.constant_initializer(0.25))
...
def cls_ohem(cls_prob, label):
    ...
    num_row = tf.compat.v1.to_int32(cls_prob.get_shape()[0])
    #row = [0,2,4.....]
    row = tf.range(num_row)*2
    indices_ = row + label_int
    label_prob = tf.squeeze(tf.gather(cls_prob_reshape, indices_))
    loss = -tf.compat.v1.log(label_prob+1e-10)
