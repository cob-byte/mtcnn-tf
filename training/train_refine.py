#coding:utf-8
#You only need to modify the model path (line139) and the tfrecords file (line85)
...
def train(netFactory, modelPrefix, endEpoch, dataPath, display=200, baseLr=0.01, gpus=""):
    ...
    dataset_dir = os.path.join(dataPath, 'all_48_occ.tfrecord')
    ...
    #     pos_dir = os.path.join(dataPath, 'all_24_occ.tfrecord')
    ...
    label = tf.placeholder(tf.float32, shape=[config.BATCH_SIZE], name='label') #eippo
    ...
    sess.run(init)
    #saver =tf.train.import_meta_graph(rootPath+'/tmp/model/pnet/pnet-30.meta')
    saver.restore(sess,tf.train.latest_checkpoint(rootPath+'/tmp/model/onet'))
...
if __name__ == "__main__":
    ...
    modelPrefix = os.path.join(rootPath, "tmp/model-refine/%s/%s"%(args.stage, args.stage))
